...
func (x *ReportEntry) ImportDynamoRecord(record *events.DynamoDBEventRecord) error {
	...
	if err != nil {
		return golambda.WrapError(err, "Failed to parse createdAt of DynamoRecord").
			With("record", record)
	}
	x.CreatedAt = v
...
func (x *ReportEntry) Import(report *deepalert.Report) error {
	...
	if err != nil {
		return golambda.WrapError(err, "Failed to marshal report.Result").With("report", report)
	}
	x.Result = string(raw)
...
func (x *ReportEntry) Export() (*deepalert.Report, error) {
	...
	if err := json.Unmarshal([]byte(x.Result), &report.Result); err != nil {
		return nil, golambda.WrapError(err, "Failed to unmarshal reprot.Result").With("entry", *x)
	}
